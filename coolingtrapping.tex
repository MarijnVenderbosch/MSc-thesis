...
\begin{equation}\label{eq:MagicCrossing}
    \operatorname{Re}\left\{\alpha(\lambdaup-\lambdaup_{\ket{{}^1S_0})})\right\} = 
    \operatorname{Re}\left\{\alpha(\lambdaup-\lambdaup_{\ket{{}^3P_0})})\right\}
\end{equation}

Calculating the magic crossing point of \cref{eq:MagicCrossing} is outside the scope of this thesis, but we will show the final result as found by \cite{Boyd2007} in \cref{fig:BoydMagic}. Plotting the polarizability for both clock states as a function of wavelength and one findes a \textit{magic} crossing point around 813 nm. By trapping both qubit states in the same potential, one can coherently drive the qubits to any point on the Bloch spheres. 
...
\begin{figure}
    \centering
    \includegraphics[width=.4\linewidth]{figures/2groundTweezer.pdf}

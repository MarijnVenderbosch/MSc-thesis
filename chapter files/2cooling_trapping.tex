...
Finally \cref{sec:Sr}, is on laser cooling for Sr, the element we want to use in the end. 
...
We have plotted the result of \cref{eq:OpticalMolasses} in \cref{fig:SpatialDependencePlots} as a function of velocity in units of $\gamma / k$, such that it is dimensionless. Contributions of both beams, as well as their total force, are shown in units of $\hbar k \gamma$.
...
To get a feeling for the numbers: for Rb-85 the Doppler temperature is $146$ $\mu$K, whereas for Sr this is about an order of magnitude lower. 
...
\begin{figure}
    ...
    \caption{\textbf{a)} Optical molasses: contributions from the lasers in both directions $F^+$ and $F^ -$ are shown, as well as the total force. Spatial force of the MOT beams and magnetic field gradient. Forces are made dimensionless by dividing by $\hbar k \gamma$.}
...
In the next section, we will derive a useful expression for the dipole potential of parameters that can be experimentally controlled. 
...
In the experimental section of this work we used Rb, because we had lasers available to get started quickly with loading optical tweezers. 
Also, it has a high vapor pressure, making it easy to load a \ac{MOT} with.
...
Rb only has one valence electron, which has ground state $L=0, S=1/2$ which means $J=1/2$ such that we can label this state as $5^{1/2}S_{1/2}$.
The closest excited state is the $L=1$ (D-line), which is split in fine splitting levels $J=1/2$ (D$_1$) or $J=3/2$ (D$_2$).
The splitting is quite substantial: the D$_2$ line has a wavelength of $\sim780$ nm whereas D$_1$ $\sim$ 795 nm. 
...
For Rb-85, the nuclear spin $\mathbf{I}=5/2$, which will couple with $\mathbf{J}$, leading to a total atomic angular momentum $\mathbf{F} = \mathbf{J} + \mathbf{I}$ and $F$ can take for the ground state values $F=2,3$ and for the excited state $F=1,2,3,4$ \cite{Steck2008}. Because of selection rules, the transitions between hyperfine levels have to satisfy 

\begin{equation}
    \Delta F = -1, 0, +1
\end{equation}

This means that the $F = 3 \rightarrow F'=4$ is a special transition: it is \textit{closed}.
This line is used, because it is an almost closed transition, meaning once excited to $F'=4$ by some cooling laser, selection rules forbid the atom from decaying back to anything else than $F'=3$. This cycle can be repeated millions of times per second, leading to efficient cooling.

While the laser is tuned very closely to the cooling transition, from \cref{eq:ScatteringFrequency}, even off-resonant, so near the tails of the Lorentzian, there is still a slim probability that the atoms will be excited to $F'=3$ instead of $F'=4$.
Consequently, they can decay to $F=2$, leaving the closed loop.
...
\begin{figure}
    ...
    \caption{(Hyper)-fine level structure of the Rb-85 ground and excited states. Also shown: cooling/trapping transition as well as repump.}
    \label{fig:D2line}
\end{figure}
...
Because we are only interested in the ground state Stark shift, we will limit ourselves to excited states $e_j$ coupled to the ground state $g$, such that the ground state dipole potential is
...
Therefore, we used a trap depth of $U_{\text{dip}} /k_b \sim 1$ mK for the tweezer, here $k_b$ denotes Boltzmann's constant, or $U_{\text{dip}} / h \sim 20$ MHz.
...
The lower end of this spectrum gives deeper traps from \cref{eq:Stark} for the same laser power, but it will also be harder to separate atomic fluorescence at $\sim 780$ nm from the tweezer light, which is easier for larger detuning.
We settled at a compromise of $820$ nm for the optical tweezer, which is very close to the wavelength used for Sr as well (see \cref{sec:Sr}). As for the waist, as typical waist for this wavelength is $\sim 0.8$ $\mu$m, as more thoroughly described in \cref{ch:tweezer}. Plugging all of this in \cref{eq:RbTrapDepth} we find we need about $1.2$ mW of laser power. 
...
Because of the one valence electron of Rb, its level structure is relatively straightforward. 
However, also group 2 atoms, also called alkali-earth (and similar atoms like Yb) are possible candidates for quantum computing and simulation applications.
...
Because of the lack of hyper-fine structure, this isotope is a bit easier to work with, but in principle, one can use all isotopes in the same machine as the energy splitting between the isotopes is in the hundreds of MHz regime, easily covered by AODs and EOMs \cite{Stellmer2013}.
...
The advantage of using a glass cell is that it allows for the maximum amount of optical access, a valuable asset when basically every operation performed on the atoms is done using laser beams.
...
For example, the van der Waals interaction coefficient scales as $\propto n^{11}$ \cite{Gallagher1994}. Other qubit implementations than the one above are possible as well.
...
To use the states in \cref{eq:QubitManifold}, they should both be trapped by exactly the same dipole potential.
Because the trapping potential is wavelength dependent, in general, the trapping potential will not be the same for both qubit states when using the same trapping wavelength, because the polarizability in \cref{eq:DipoleForce}: is wavelength dependent: $\alpha = \alpha(\omega) = \alpha(\lambdaup)$, meaning that the transition frequency between the qubit states would change as a function of the position within the potential which is unfavorable. 
...
This this way, the differential Stark shift, that is the difference in AC Stark shift between the states, is minimized:

\begin{equation}\label{eq:DifferentialStarkShift}
    \hbar \delta \nu = \delta U_1 (\mathbf{E}, \omega_1) - \delta U_0(\mathbf{E}, \omega_0)
\end{equation}

\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{figures/MagicTrapping.pdf}
    \caption{Caption}
    \label{fig:my_label}
\end{figure}

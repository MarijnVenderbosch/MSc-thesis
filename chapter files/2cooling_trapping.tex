...
where $k_b$ is Boltzmann's constant. Apparently, this cooling limit is only dependent on the linewidth of the transition $\gamma$, apart from physical constants. This result will be used later in \cref{sec:Sr}. For Rb-85 the Doppler temperature is $146$ $\mu$K. Sr MOTs are typically orders of magnitude colder.
...
The polarizatibility was derived classicaly in \cite{Grimm2000}, resulting in the following trap depths and scattering rates.
...
and $m_e$ is the electron mass. \Cref{eq:DipoleClassicalResult} is valid for the case $\delta \ll \omega, \Gamma \ll \omega$.
From \cref{eq:DipoleClassicalResult} the relation these two quantities is:

\begin{equation}\label{eq:RelationUG}
    \hbar \Gamma_{sc} =\Gamma U_{\text{dip}}/\delta
\end{equation}
...
While the classical treatment is fairly accurate for alkali-metals like Rb \cite{Grimm2000}, we show a more general semi-classical treatment below as well.

\subsection{Semi-Classical}

In the semi-classical treatment a classical light field is considered.
...
The atom has two quantized eigenstates: $\ket{g}$ (ground) with energy $\hbar \omega_g$ and $\ket{e}$ (excited) with $\hbar \omega_e$, see \cref{fig:2LevelAtom}. 
...
Similar to the classical result, the light shift is proportional to the light intensity over the detuning: $\Omega^2 / \delta \propto I / \delta$.
...
Because of the light shift, the eigenstates of the system are shifted, the new eigenstates can be found in appendix \ref{ch:DressedApproach}.


\subsection{Laser cooling in Practice}\label{sec:Practice}

In order to reliably trap atoms near the bottom of the tweezer, the dipole potential should be significatly deeper 

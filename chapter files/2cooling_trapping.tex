...
\Cref{sec:PracticeRb} discusses laser cooling on a more practical level for Rb, the element that we mainly used for this work. 
...
Because of the Doppler effect, the atom \lq sees' a slightly higher frequency $\delta'$ when moving towards the light with a velocity $v$: $\delta'=\delta+k v$ and the laser may become resonant, causing the atom to absorb a photon absorbing momentum $\hbar k$ and promoting an electron to the excited state $\ket{e}$.
...
The force will have a contribution for positive velocities $F^+$ as well as for negative velocities $F^-$, making the total force $F^+ + F^-$ from both contributions of \cref{eq:ScatteringFrequency} \cite{Kowalski2010}
...
\centering
	\begin{subfigure}{.38\textwidth}
		\centering
		\includegraphics[height=4.6cm]{figures/2LevelAtom.pdf}
		\caption{}
		\label{fig:2LevelAtom}
	\end{subfigure}
	\begin{subfigure}{.61\textwidth}
		\centering
		\includegraphics[height=4.6cm]{figures/Molasses.pdf}
		\caption{}
		\label{fig:MOTPlots}
	\end{subfigure}
...
\end{equation}
Here, $\mu' = (g_e m_e-g_g m_g)\mu_B$ is the effective magnetic moment for the transition \cite{Kowalski2010}. 
...
For $\delta <0$ (\lq red detuned'), the ground state light state is negative. 
...
Though, in principle, one can use all isotopes in the same machine as the energy splitting between the isotopes is in the hundreds of MHz regime, easily covered by AOMs and EOMs \cite{Stellmer2013}.
...
\begin{itemize}
	...
	This is useful to slow down the hot atomic beam coming from the oven, as well as catching the atoms in a so-called blue \ac{MOT} with \lq hot' temperatures of $\sim$ 1 mK.
	...
	It will turn out that this feature can be put to good use in quantum computers as well, and we will use it to coherently \lq drive' the qubits between the qubit basis states using this clock transition. 
...
In the glass cell, all the magic happens: the magneto optical traps using the blue as well as red transitions, dipole traps, as well as all the qubit operations (coupling, pulses, readout) happen in this cell.
...
As a result of these exaggerated electron orbit sizes, neighboring atoms can \lq feel' each other.

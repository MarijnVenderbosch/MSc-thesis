...
\begin{figure}
    \centering
    \includegraphics{figures/crossAOD.pdf}
    \caption{Two AODs in crossed configuration, used to make a 2D spot array. Figure from \cite{Madjarov2021}.}
    \label{fig:my_label}
\end{figure}
...
\begin{equation}
    \phi(V) = \frac{2\pi}{\lambdaup} (n_e - n_0) t
\end{equation}
...
\begin{figure}
    \centering
    \includegraphics[width=.65\linewidth]{figures/SLMgeometry.pdf}
    \caption{SLM plane with phases $\phi_j(x,',y',z=0)$ for pixel $j$, as well as the focal plane or Fourier plane with trap coordinates for trap $m$ of $(x_m,y_m,z_m)$.}
...
This type of algorithm typically converges after a couple tens of steps. The algorithm was implemented in Python by Ivo Knottnerus as part of his PhD research. Because the SLM has some $\sim 2$ million pixels, evaluating \cref{eq:Vm,eq:Argument} can be time consuming depending on the desired pattern and is therefore executed on a graphics card using the \textit{PyOpenCL} library. 

The hologram is sketched in \cref{fig:GerschbergSaxton}

\begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{figures/WeightedGerschbergSaxton.pdf}
    \caption{The GSW algorithm visualized.}

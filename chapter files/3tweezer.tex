...
In practice, it is not practical to use \cref{eq:FourierBesselAperture} to fit data and it is more convenient to use a simpler function. 
...
Optimizing for maximum trap frequencies is useful because from \cref{eq:TrapFrequencies}, higher trap frequencies translate to deeper as well as physically smaller optical dipole potentials\footnote{Trap frequencies can also be measured experimentally to perform aberration correction on tweezer arrays \cite{Ebadi2021}.}.
The optimum $w_i/R$ ratio in terms of maximizing \cref{eq:TrapFrequencies} was found by \cite{Madjarov2021} to be $w_i\simeq R$, which we will use for the remainder of this work. 
...
\end{equation}
For plane wave illumination, or $E(r')$ equal to a constant, \cref{eq:FresnelCylindrical} yields the \ac{PSF} of a perfect lens.
...
To fully illuminate the SLM, which has dimensions of $\sim 15.4 \times 9.6$ mm (\cref{table:SLMspecs}), a fiber collimator\footnote{\textit{Schäfter + Kirchhoff} 60FC-L-0-M60-10.} with a clear aperture of 17 mm was used ($f=60$ mm, $M^2 < 1.05$, $1/e^2$ beam radius 4.8 mm).
...
The axial intensity distribution is thus not diffraction-limited, though it is fairly close.
Most likely, the discrepancy is caused by an aberration, for example a slight tilt in one of the optical components. 

...
\Cref{sec:DiffractionLimit} discusses we intend to do this using a microscope objective, as well as the fundamental limit of the smallest achievable spot size: the diffraction limit.
...
\section{Diffraction Theory}\label{sec:DiffractionLimit}

Because of the wave character of light, there is a fundamental limit to the smallest spot size one can focus this wave, even in the absence of aberrations.
Consider a circular lens with aperture radius $R$ and focal length $f$.
We assume it is a thin lens positioned at $z=0$.
The complex light field just before and just after the lens are denoted $E_i(x',y')$ and $E_l(x',y')$ respectively, see \cref{fig:LensAperture}.
...
\begin{figure}
	...
	The input field is $E_i(x',y')$ which is only defined on the aperture. }
...
\end{equation}
Hidden in the term $E_i$ is the aperture function, which is zero for $x'^2+y'^2>R^2$ or in cylindrical coordinates $r>R$.
...
\end{equation}
Which might look like an impossible integral to solve, which it is analytically. 
However, in the next sections it becomes clear that for specific cases the integral simplifies greatly.  

\subsection{In the Focus of a Lens}
...
About this 'input' function.
For optical tweezers, we shine a laser beam into the lens.
Assuming the beam is Gaussian with 'input' waist $w_i$, the input light field is up to a normalization constant $A$
...
\begin{figure}
    \centering
    \includegraphics[width=.61\linewidth]{figures/GaussianPotential.pdf}
...
In producing the perfect \ac{PSF} we assumed plane wave illumination ($w_i \gg R$, but in reality this is impractical: assuming the beam is aligned with the center of the aperture, the fraction of optical power $P/P_0$ traversing the aperture as a function of the incident waist $w_i$ is
...
\centering
	...
	\end{subfigure}
	\caption{Radial intensity profile \textbf{a)} prior aberration correction yielding a fit $z_R \sim 6.2$ $\mu$m and \textbf{b)} after aberration correction: $z_R \sim 3.5$ $\mu$m. Also shown: axial intensity data as well as the theory result.}

...
In the lab, two methods to do this are used. 
The first way is to send several laser beams under slightly different angles into the objective.
...
Using an SLM for tweezer arrays has the advantage that it is possible to adapt the hologram on the fly, correcting for variation within trap depths of the array \cite{Nogrette2014}. 
...
Because we only computed a small number of holograms, we simply used \cref{eq:Vm}, but in a newer version of the code the (inverse) FFTs are used. 
...
As the equations describing light propagation are time-symmetric, the phase mask is found by considering the reverse direction, by considering $M$ coherent light sources that are virtually positioned on the location of the spots. 
...
\begin{enumerate}
    ...
    \item From the amplitudes $V_m^{k+1}$, the new weight factors $w_m^{k+1}$ and phases $\theta_m^{k+1}$ are computed as \cite{Leseleuc2018}
...
\end{enumerate}
The electro-optical response is slightly different between individual SLM units and is therefore always performed. 
...
We estimate this power loss by reflecting a Gaussian beam $G(x,y)$ having waist $w=w(z)$\footnote{Because the beam is collimated, we assume the waist is independent of $z$.} and power $P_0$ off of a rectangular aperture of dimensions $(2S_x, 2S_y)$ where $S_{x,y}$ are the semi-widths of the aperture, see \cref{fig:Rectangular}.
\begin{figure}[]
    \centering
...
This collimator yields a beam with waist $w=4.7$ mm, about the size of the short semi-width $S_y = 4.8$ mm of the rectangular SLM.
...
\begin{figure}
    ...
    \textsf{\textbf{b)}} Aberration remaining after subtracting the Mituotoyo factory correction.
...
\pi (\theta_{\text{m}}/\theta_{\text{m,max}})/2\}$ where $\theta_m$ is the deflection angle from the zeroth order for spot $m$ in the array and $\theta_{m,\text{max}}$ maximum deflection angle \cite{Ebadi2021}.

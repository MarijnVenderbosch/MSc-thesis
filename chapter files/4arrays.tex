...
In \cref{sec:SLM} we will introduce the working principle and some measurements on the SLM and in \cref{sec:GSW} we will see how to compute the hologram needed.
...
A spatial light modulator is in essence a mirror that can imprint a \ac{CGH} onto a beam of light, allowing for spatial light modulation.
...
The the refractive $\Delta n(V)$ and therefore the light retardation $\phi(V)$ (\cref{eq:ElectroOpticResponse}) is not necessarily linear as a function of the applied voltage V. 
...
Measuring the electro-optic response is impossible because the light field oscillates with frequencies in th THz regime.
...
Assuming amplitude modulation between different gray levels is negligible and the SLM only modulates phase, the electric field just after the SLM is proportional to the phase transmittance given by summing over the gratings $m$ \cite{Zhang1994}
...
For our 8-bit SLM, this amounts to looping over 255 gray values. For each hologram, separate the multiple diffraction orders using a $f=400$ mm lens (such that the spacing between the various orders is sufficient) and an aperture and record the power in the first order using a power meter.
...
\subsection{Light Propagation}\label{sec:PropagationDerivation}

N
Labeling the pixels $j$, the phase retardance is thus $\phi_j(x_j,y_j)$ where $(x,y)$ are Cartesian coordinates in the plane of the SLM.

Now, consider a laser with intensity distribution $|E_i|^2$ impinging on the SLM, it will pick up a phase factor such that after the SLM, the field is described as $E_i e^{i\phi(x',y')}$ this is shown in \cref{fig:SLMgeometry}. 
The field will interfere with itself, until at infinity (or in the focal plane of a lens) the distribution evolves to $|E_f|^2$.
...
\begin{figure}
    ...
    \caption{Field with intensity distribution $|E_i|^2$ impinging on the SLM, which due to its finite size acts as an aperture.
